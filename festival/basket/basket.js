/*
 * Basketturnering 3x3 — anmälan
 * Stockholms Moské Festival 2026
 *
 * Samma formulärmönster som fotboll.js: namn, telefon, e-post, lagnamn,
 * consent. Postar till Apps Script med { action: 'submitBasket', data: { ... } }.
 */
(function () {
  'use strict';
  ...
  function buildForm() {
    ...
    var form = el('form', { id: 'basket-form', novalidate: 'novalidate' });
  ...
  function handleSubmit(e) {
    ...
    sendBasket(payload, function (result) {
      if (result && result.ok) {
  ...
  function sendBasket(payload, callback) {
  ...
  function showSuccess(form, teamName, name) {
    ...
    success.appendChild(el('p', {
      text: 'Vi har tagit emot anmälan för ' + (teamName ? '“' + teamName + '”' : 'ditt lag') +
            '. Du får en bekräftelse via SMS när platsen är godkänd, tillsammans med Swish-uppgifter för anmälningsavgiften (400 kr).'
    }));
